...
const {
    ...
    loading: pageLoading,
    refresh: refreshPage
} = await useAPILazyAsyncData<PageData | null>('admin-status-page-config', async () => {
...
const config = computed(() => pageData.value?.config ?? null)
...
const groupCreateForm = reactive<CreateGroupSchema>({
    name: '',
    sort_order: 0
})
...
<template>
    <UDashboardPanel>
        <template #header>
            <DashboardPageHeader
                ...
                description="Configure public status page"
            />
        </template>
        ...
        <template #body>
            <DashboardPageBody>
                ...
                <div v-else-if="!config" class="text-center py-12">
                    <UEmpty icon="i-lucide-file-x" title="Configuration not found" description="Failed to load status page configuration." variant="naked" />
                </div>
    ...
    <!-- Create Link Modal -->
    <DashboardModal
        v-model:open="showCreateLinkModal"
        title="Link Monitor"
        icon="i-lucide-link"
    >
        <UForm :schema="createLinkSchema" :state="linkCreateForm" class="space-y-4" @submit="handleCreateLink">
            <UFormField label="Monitor" name="monitor_id" required>
                <USelect
                    v-model="linkCreateForm.monitor_id"
            ...
            <UFormField label="Group" name="group_id">
                <USelect
                    v-model="linkCreateForm.group_id"
                    :items="groupOptions"
    ...
    <!-- Edit Link Modal -->
    <DashboardModal
        v-model:open="showEditLinkModal"
        :title="`Edit Link: ${selectedLink?.monitor_name}`"
        icon="i-lucide-pencil"
    >
        <div class="space-y-4">
            <UFormField label="Group">
                <USelect
                    v-model="linkEditForm.group_id"
                    :items="groupOptions"
                    placeholder="Ungrouped"
                    class="w-full"
                />
