...
type DragPayload =
    | { type: 'available-monitor'; id: number; monitor: Monitor }
...
// --- Available monitor drag ---

function onAvailableDragStart(monitor: Monitor, event: DragEvent) {
    event.dataTransfer?.setData('text/plain', JSON.stringify({ type: 'available-monitor', id: monitor.id }))
    event.dataTransfer!.effectAllowed = 'copy'
    setDragPayload({ type: 'available-monitor', id: monitor.id, monitor })
}
...
function onLinkDragOver(group: EditorGroup, link: EditorLink, index: number, event: DragEvent) {
    ...
    event.preventDefault()
    event.dataTransfer!.dropEffect = dragState.value.type === 'available-monitor' ? 'copy' : 'move'
...
function onLinkOrBodyDrop(group: EditorGroup, event: DragEvent) {
    ...
    if (dragState.value.type === 'available-monitor') {
        // Append monitor to the target group to avoid sort_order conflicts before the link exists
        const monitor = dragState.value.monitor
        const targetLinks = editorGroups.value.find(g => g.id === targetGroupId)?.links ?? []
    ...
    } else if (dragState.value.type === 'link') {
...
function getStatusIconName(status: string | null | undefined): string {
    switch (status) {
        case 'up': return 'i-lucide-check-circle'
        case 'down': return 'i-lucide-x-circle'
        case 'degraded': return 'i-lucide-alert-triangle'
...
<template>
    <div class="flex flex-col lg:flex-row gap-4">
        <!-- Left sidebar: available monitors -->
        <UCard class="lg:w-72 border-slate-800 bg-slate-900/60 flex flex-col shrink-0">
            <template #header>
                <div class="flex items-center justify-between">
                    <h3 class="font-semibold text-white">Available Monitors</h3>
                    <UBadge variant="soft" color="neutral">{{ availableMonitors.length }}</UBadge>
                </div>
            </template>
            ...
            <div v-else-if="availableMonitors.length === 0" class="text-sm text-slate-400 py-4 text-center">
                All monitors are already on the status page.
            </div>
            ...
            <div v-else class="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                <div
                    ...
                    :key="monitor.id"
                    draggable="true"
                    class="flex items-center gap-2 p-2 rounded-lg border border-slate-800 bg-slate-900/80 hover:border-slate-700 hover:bg-slate-800/50 cursor-grab active:cursor-grabbing transition-colors"
                    @dragstart="onAvailableDragStart(monitor, $event)"
                    @dragend="onDragEnd"
                >
                    <UIcon name="i-lucide-grip-vertical" class="text-slate-500 size-4 shrink-0" />
                    ...
                    <UButton
                        ...
                        size="xs"
                        class="shrink-0"
                        @click="emit('create-link', { monitor_id: monitor.id, group_id: null, display_name: null, sort_order: ((editorGroups.find(g => g.id === null)?.links ?? []).length ? Math.max(...(editorGroups.find(g => g.id === null)?.links ?? []).map(l => l.sort_order)) + 1 : 0) })"
                    />
                </div>
            </div>
        </UCard>
        ...
        <div class="flex-1 space-y-3 min-w-0">
            ...
            <div class="space-y-3">
                ...
                <template v-for="(group, groupIndex) in editorGroups" :key="group.id ?? 'ungrouped'">
                    <UCard
                        class="border-slate-800 bg-slate-900/60 overflow-hidden"
                    ...
                    >
                        ...
                        <!-- Group body (drop zone + links) -->
                        <div
                            class="min-h-[80px] space-y-1"
                        ...
                        >
                            ...
                            <div
                                v-if="group.links.length === 0 && !props.loading && dragState?.type !== 'group' && dragOverGroupId === group.id"
                            ...
                            <template v-for="(link, linkIndex) in group.links" :key="link.id">
                                ...
                                <div
                                    v-if="dragState?.type !== 'group' && dragOverGroupId === group.id && dragOverLinkIndex === linkIndex && dragIndicatorPosition === 'before'"
                                    class="h-1 rounded bg-primary-500/70 my-1"
                                />
                                ...
                                <div
                                    ...
                                    draggable="true"
                                    class="flex items-center gap-3 p-2 rounded-lg border border-slate-800 bg-slate-900/40 hover:border-slate-700 hover:bg-slate-800/40 cursor-grab active:cursor-grabbing transition-colors"
                                ...
                                >
                                    ...
                                    <UIcon
                                        :name="getStatusIconName(link.monitor?.latest_check?.status)"
                                        class="size-4 shrink-0"
                                        :class="getStatusColor(link.monitor?.latest_check?.status)"
                                    />
                                    ...
                                    <span
                                        ...
                                        class="text-xs text-slate-500 hidden sm:inline shrink-0"
                                    >
                                ...
                                <div
                                    v-if="dragState?.type !== 'group' && dragOverGroupId === group.id && dragOverLinkIndex === linkIndex + 1 && (linkIndex === group.links.length - 1)"
                                    class="h-1 rounded bg-primary-500/70 my-1"
                                />
                            </template>
                            ...
                            <div
                                v-if="dragState?.type !== 'group' && dragOverGroupId === group.id && dragOverLinkIndex === group.links.length && group.links.length > 0 && dragIndicatorPosition === 'before'"
                                class="h-1 rounded bg-primary-500/70 my-1"
                            />
                        </div>
                    </UCard>
